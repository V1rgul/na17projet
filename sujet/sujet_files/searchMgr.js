...
var searchMgr = {
	...
	fStrings : ["Annuler","Annuler la recherche",                                 //0
            ...
            "1 page trouvé","%s pages trouvés",                                 //4
            "Précisez votre recherche...","Termes recherchés :",                //6
            "Précédent","Occurrence précédente",                                //8
            "Suivant","Occurrence Suivante",                                    //10
            "Page Précédente","Page Suivante",                                  //12
            "Liste","Afficher/cacher la liste des pages trouvées",              //14
            "%s occurrences","1 occurrence",                                    //16
            "plus de 8 occurrences","%s"],                                      //18
...
/* ========== Public functions ============================================== */
	...
	init: function(pPathSchBox,pPathResBox,pOpt){
		...
		if (!vSchBox) return;
		this.fSearchCmds = this.xAddElt("div",vSchBox,"schCmds");
	...
	onLoad: function(){
		...
		try{
			...
			this.fSearchPropose = this.xAddElt("div",this.fSearchCmds,"schPropose schProp_no");
			
			var vResultFrame = this.xAddElt("div",this.fSearchRes,"schResFrame" + (this.fUnifiedNav? " schUnifiedNav":""));
			var vResultList = this.xAddElt("div",vResultFrame,"schResList");
			this.fResultScroll = this.xAddElt("div",vResultList,"schResListSrl");
			...
			var vSchHitBox = this.xAddElt("div",vResultFrame,"schHitBox");
			this.fHitLbl = this.xAddElt("span",vSchHitBox,"schHitLbl");
			this.fBtnPrvHit = this.xAddBtn(vSchHitBox,"schBtnPrvHit",this.fStrings[8],this.fStrings[9]);
			this.fBtnPrvHit.onclick = this.sPrvHit;
			this.fHitCnt = this.xAddElt("span",vSchHitBox,"schHitCnt");
			...
			var vSchPageBox = this.xAddElt("div",vResultFrame,"schPageBox");
			this.fSearchLbl = this.xAddElt("span",vSchPageBox,"schResLbl");
			this.fSearchCnt = this.xAddElt("span",vSchPageBox,"schResCnt");
			if (!this.fUnifiedNav){
...
/* === Callback functions =================================================== */
	...
	xUpdateUi: function(){
		...
		if (this.fResult && this.fResult.length > 0){
			...
			var vRoot = scPaLib.findNode("des:.tplFra");
			if (!vRoot) return;
	...
	xHighlight: function(pRoot, pStr){
		...
		for (i = 0; i<vTextNodes.length;i++){
			...
			if (vHits.length>0){
				...
				var vBkExtra = scPaLib.findNode("anc:.cbkClosed",vTxtNode);
				if(vBkExtra && !scPaLib.findNode("anc:h\\d",vTxtNode)) {
					var vTgl = scPaLib.findNode("des:a", vBkExtra);
...
searchMgr.ListResultManager.prototype = {
	...
	buildResult : function(pResult) {
		...
		this.fDedupeResults = [];
		var vRoot = searchMgr.xAddElt("ul",this.fRoot,"mnu_root");
		for (var i = 0; i < this.fResult.length; i++){
			...
			if(vPageRes) {
				...
				vPageRes.fLbl = searchMgr.xAddElt("li",vRoot,"schPgeBk mnu_sel_no schPgeRank_"+vResult.cat);
				...
				searchMgr.xAddElt("span",vPgeBtn,"schPgeRank").innerHTML = "<span>" + vRankText + "</span>";
				...
				if(vPageRes.urls.length > 1) {
					...
					vTglBtn.fUl = searchMgr.xAddElt("ul",vRoot,"schParentList schParentList_c");
					var vParentArr = vPageRes.parents;
					for(var j = 0; j < vParentArr.length; j++){
					    vPageRes.fLiParentBk[j] = searchMgr.xAddElt("li",vTglBtn.fUl,"mnu_sel_no");
					    ...
					    if(vParentArr[j] != undefined){
						    for(var k = 0; k < vParentArr[j].length; k++){
						    	var vParentLabel = searchMgr.xAddElt("span",vParentLnk,"schParentLabel");
...
searchMgr.MenuManager = function (pRoot, pOutline, pOpt) {
	try{
		...
		if (this.fOpt.contextRoot) this.fContext = searchMgr.xAddElt("span",this.fOpt.contextRoot,"ctx_root");
...
searchMgr.MenuManager.prototype = {
	...
	buildMenuScroller : function() {
		...
		this.fSrlUp = searchMgr.xAddElt("div", vFra, "mnuSrlUpFra", null, null, this.fRoot);
		...
		this.fSrlDwn = searchMgr.xAddElt("div", vFra, "mnuSrlDwnFra");
		...
		for (i=0; i< pRoot.fSrc.children.length; i++){
			...
			if (!this.fFilter && !vChi.prn || this.fFilter && vChi.vis){
				...
				if (vTyp == "b"){
					...
					vUl = searchMgr.xAddElt("ul",vChi.fLi,"mnu_sub mnu_sub_c",true);
	...
	buildMenuEntry : function(pParent, pSrc, pHidden) {
		...
		vLi = searchMgr.xAddElt("li",pParent,vCls,pHidden);
		vDiv = searchMgr.xAddElt("div",vLi, "mnuLbl "+vCls);
		vLnk = searchMgr.xAddElt("a",vDiv,"mnu_i mnu_lnk");
		if (pSrc.url) {
	...
	rebuildMenu : function() {
		...
		var vRootUl = searchMgr.xAddElt("ul",this.fRoot,"mnu_root mnu_sub mnu_sch_no");
