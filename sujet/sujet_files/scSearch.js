...
scServices.scSearch = scOnLoads[scOnLoads.length] = {
	...
	query: function(pQryLst, pOpt){
		...
		for (var i=0; i<vQryLst.length; i++) {
			var vQry = vQryLst[i];
			var vQryOpt = {matchExact:false,catMask:null,returnResultSet:true};
	...
	find: function(pId, pStr, pOpt){
		...
		if (!vIdx) return null;
		var vOpt = {matchExact:false,catMask:null,returnResultSet:false};
		...
		var vWrdResult = {};
		var vRawResults = [];
		var vTokens = this.xBuildTokens(vIdx,pStr);
		for (i = 0; i < vTokens.length; i++){
			vToken = vTokens[i];
			vWrdResult = this.xFindWord(vIdx,vToken.wrd,{matchExact:vToken.exact||vOpt.matchExact,catMask:vOpt.catMask});
			if (vWrdResult) {
	...
	propose: function(pId, pWrd, pOpt){
		...
		var vOpt = {async:false,matchStartOnly:true,maxNum:20,catMask:null};
		if (typeof pOpt != "undefined"){
			...
			if (typeof pOpt.catMask != "undefined") vOpt.catMask = pOpt.catMask;
		}
		...
		if (vOpt.matchStartOnly){
			var vFound = false;
			for (var i = 0; i<vIdx.fSortedWords.length; i++){
				if(vIdx.fSortedWords[i].wrd.indexOf(vWrd)==0){
					vRet.push(vIdx.fSortedWords[i]);
					vFound=true;
					if (vOpt.maxNum>0 && vRet.length==vOpt.maxNum) return null;
				} else if (vFound) break;
			}
		} else {
			for (var i = 0; i<vIdx.fSortedWords.length	;i++){
				if(vIdx.fSortedWords[i].wrd.indexOf(vWrd)>=0){
	...
	xFindWord: function(pIdx, pWrd, pOpt){
		...
		var vRegFind = new RegExp("^"+(vMatchExact?"":"[^\\t]*")+pWrd+(vMatchExact?"":"[\\S]*")+"\t[^\\n]*","gm");
//		var vRegFind = new RegExp("^"+pWrd+(vMatchExact?"":"[\\S]*")+"\t[^\\n]*","gm");
//		var vRegFind = new RegExp("^[^\\t]*"+pWrd+(vMatchExact?"":"[\\S]*")+"\t[^\\n]*","gm");
	...
	xLoadIndex: function(pUrl,pAsync){
		...
		var vIdx = this.fIdxs[pUrl] = {id:pUrl,fLoaded:false};
		var vReq = null;
		...
		function iSetupIndex(pIdx,pRaw) {
			...
			try {
				var vRegHead = new RegExp("^([^\\n]*)\\n([^\\n]*)\\n([^\\n]*)");
				var vHeadStr = vRegHead.exec(pRaw);
				pIdx.fPgeIdx = scServices.scSearch.xDeserialiseObjJs(vHeadStr[1]);
				pIdx.fCatIdx = scServices.scSearch.xDeserialiseObjJs(vHeadStr[2]);
				pIdx.fParams = scServices.scSearch.xDeserialiseObjJs(vHeadStr[3]);
				pIdx.fRaw = pRaw;
				pIdx.fLoaded = true;
	...
	xSortIndex : function(pIdx){
		try {
			...
			pIdx.fSortedWords = [];
			for (var i = 3; i<vLines.length; i++){
				var vLine = vLines[i].split("\t");
				pIdx.fSortedWords.push({wrd:vLine[0],num:vLine.length-1});
			}
