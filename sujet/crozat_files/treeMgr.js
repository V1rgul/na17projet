...
/*02*/              "Fermer","Femer tous les nœuds",
/*04*/              "Chercher","Activer/désactiver la recherche de nœuds",
/*06*/              "Chercher un nœud :","Tapez votre recherche",
/*08*/              "Cherche textuel dans les titres des nœuds (trois charactères minimum)","Veuillez précisez votre recherche.",
...
treeMgr.onLoad = function(){
	scCoLib.util.log("treeMgr.onLoad");
	for (var i=0; i<this.fTrees.length; i++){
		var vTree = this.fTrees[i];
...
treeMgr.makeVisible = function(pNode){
	scCoLib.util.log("treeMgr.makeVisible");
...
treeMgr.toggleSearch = function(pTree){
	scCoLib.util.log("treeMgr.toggleSearch");
...
treeMgr.toggle = function(pBtn,pDeep){
	scCoLib.util.log("treeMgr.toggle");
...
treeMgr.previous = function(pTree){
	scCoLib.util.log("treeMgr.previous");
	if (pTree.fResults.length==0) return;
...
treeMgr.xUpdateResults = function(pTree, pNode) {
	scCoLib.util.log("treeMgr.xUpdateResults");
...
treeMgr.xResetSearchResults = function(pTree) {
	scCoLib.util.log("treeMgr.xResetSearchResults");
...
treeMgr.xSearch = function(pTree, pVal) {
	scCoLib.util.log("treeMgr.xSearch");
	if (!pVal) return;
	var vCount = 0;
	pTree.fResultIdx = -1;
	pTree.fResults = [];
...
treeMgr.xBtnMgr = function(pBtn) {
	scCoLib.util.log("treeMgr.xBtnMgr: "+pBtn.fName);
	var vObj = pBtn.fObj;
	switch(pBtn.fName){
		case "treeBtnOpenAll":
